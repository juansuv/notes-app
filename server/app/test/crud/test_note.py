...
async def test_create_note_crud(async_session: AsyncSession, setup_notes_data):
    ...
    created_note = await create_note(async_session, note, setup_notes_data["user"].id)
...
async def test_get_user_notes_crud(async_session: AsyncSession, setup_notes_data):
    notes = await get_user_notes(async_session, setup_notes_data["user"].id)
    assert len(notes) > 0
    assert notes[0].owner_id == setup_notes_data["user"].id
